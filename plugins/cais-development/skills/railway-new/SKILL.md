...
- User says "create a railway project", "init", "new project" (explicit new
  project)
...
- User says "add postgres", "add a database", "add redis", "add mysql", "add
  mongo"
- User says "connect to postgres", "wire up the database", "connect my api to
  redis"
...
Railway CLI walks up the directory tree to find a linked project. If you're in a
subdirectory:
...
3. **Directory name matches existing project** → Ask: link existing or create
   new?
...
- `-n, --name` - Project name (auto-generated if omitted in non-interactive
  mode)
...
If the user has multiple workspaces, `railway init` requires the `--workspace`
flag.
...
**Inferring workspace from user input:** If user says "deploy into xxx
workspace" or "create project in my-team", match the name against the workspaces
array and use the corresponding ID:
...
`environment` skill to configure the source via staged changes API. Do NOT use
`railway add --repo` - it requires GitHub app integration which often fails.
...
- If non-standard output dir: invoke `environment` skill to set
...
- Multiple subdirs with separate `package.json` but no workspace config →
  isolated monorepo

### Scaffolding Hints

If no code exists, suggest minimal patterns from
[railpack.md](references/railpack.md):
...
For adding databases (Postgres, Redis, MySQL, MongoDB), use the `database`
skill.
