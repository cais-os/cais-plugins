...
Add official Railway database services. These are maintained templates with
pre-configured volumes, networking, and connection variables.
...
For full environment config structure, see
...
The `config.services` object contains each service's configuration. Check
`source.image` for:
...
**Important:** `serializedConfig` is the exact object from the template query,
not a string.
...
For complete variable reference syntax and wiring patterns, see
...
**Important:** Frontends run in the user's browser and cannot access Railway's
private network. They must use public URLs or go through a backend API.
...
6. Use `environment` skill to stage:
   `DATABASE_URL: { "value": "${{Postgres.DATABASE_URL}}" }`
7. Apply changes
