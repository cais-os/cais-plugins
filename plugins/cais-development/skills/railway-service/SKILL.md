...
**Note:** For creating services with local code (the common case), prefer the
`new` skill which handles project setup, scaffolding, and service creation
together.

**For GitHub repo sources:** Use `new` skill to create empty service, then
`environment` skill to configure source.repo via staged changes API.
...
**Railway Devicons:** Query `https://devicons.railway.app/{query}` for common
developer icons (e.g., `github`, `postgres`, `redis`, `nodejs`). Browse all at
https://devicons.railway.app
...
- **Create service with local code**: Use `new` skill (handles scaffolding +
  creation)
- **Configure service**: Use `environment` skill (variables, commands, image,
  etc.)
