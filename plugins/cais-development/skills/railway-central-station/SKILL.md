...
Search and browse Railway's Central Station - the community support platform for
threads, discussions, and documentation.
...
- User wants to find threads about a specific topic (deployments, databases,
  etc.)
...
- User wants to manage their Railway project - use appropriate skill (deploy,
  environment, etc.)

## Docs Search

For official Railway documentation, use the `railway-docs` skill which fetches
from `https://docs.railway.com/api/llms-docs.md`.

Central Station's `unifiedSearch` can identify document types but has limited
field access:
...
**Note**: For searching thread content, use the LLM Data Export endpoint instead
(see below) which provides full thread data.
...
Returns: questions, feedback, community, billing, bug-bounty, privacy, abuse,
templates
...
This endpoint returns all public threads with full content, useful for searching
by keywords.
...
- **No results found**: Suggest using `railway-docs` skill or creating a new
  thread
- **Technical issue found**: Use relevant skill (deploy, environment, etc.) to
  help fix it
