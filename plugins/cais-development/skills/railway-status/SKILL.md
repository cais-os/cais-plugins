...
- Detailed service configuration (builder type, dockerfile path, build command,
  root directory)
...
>
> To link an existing project: `railway link` To create a new project:
> `railway init`
...
- **Active Deployments**: any in-progress deployments (from `activeDeployments`
  field)
...
The `activeDeployments` array on each service shows currently running
deployments with their status (building, deploying, etc.).
